	...
	public void tblupdt() {
		try (
			...
			while (res.next()) {
				...
				model.setValueAt(res.getInt("id"), model.getRowCount() - 1, 0);
				model.setValueAt(res.getString("username"), model.getRowCount() - 1, 1);
				model.setValueAt(res.getString("name"), model.getRowCount() - 1, 2);
			}
		} catch (SQLException e1) {
			e1.printStackTrace();
		}
